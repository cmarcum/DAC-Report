...
ylim1 <- boxplot.stats(as.numeric(approval.time.moving.avg.table$ApprovalTimeFromPIToSO))$stats[c(1, 5)]
...
ylim2 <- boxplot.stats(as.numeric(approval.time.moving.avg.table$ApprovalTimeFromSOToDAC))$stats[c(1, 5)]
...
ylim3 <- boxplot.stats(as.numeric(approval.time.moving.avg.table$ApprovalTimeFromPIToDAC))$stats[c(1, 5)]

approval.time.from.PI.to.DAC.plot <- ggplot(approval.time.moving.avg.table, aes(factor(ApprovedMonth), ApprovalTimeFromPIToDAC)) + geom_boxplot() + 
...
on average submits `r current.dac.pi.average.dar` DAR to all DACs for `r current.dac.pi.average.project` projects.
...
During this reporting period, `r new.studies.released` new studies were released by `r dac` and a total of `r new.studies.dar` DARs were made for these studies. Study `r phs.popular.this.dac` (`r phs.popular.this.dac.study.name`) has been the most requested dataset from the `r dac` DAC with `r phs.popular.this.dac.total.request` requests.
