...
```{r fig.cap=word.cloud.fig.cap, echo=FALSE, warning=FALSE ,message=FALSE}
library(wordcloud)
...
```{r fig.cap=most.cited.fig.cap, echo=FALSE}
if (nrow(dac.study.summary.table.ordered.by.citation) > 5) {
  ...
  ggplot(cited.df, aes(x = StudyID, y = Citations, fill=StudyName)) +
    geom_bar(stat="identity") +
...
```{r echo=FALSE}
library(knitr)
#knitr::kable(timeline.summary.table)
```
