...
```{r fig.cap="Comparison of Cummulative DAR and Studies Released", echo=FALSE, warning=FALSE ,message=FALSE}
p <- ggplot(study.status.table, aes(x=Month)) +
  geom_line(aes(y=TotalRequestsCummulative, colour="Total Requests")) +
  geom_line(aes(y=StudiesReleasedCummulative, colour="Studies Released")) +
  scale_color_manual("",
                     values = c("Total Requests"="green", "Studies Released"="red"))
rect <- data.frame(xmin=as.Date(lubridate::floor_date(as.Date(params$start.date))), xmax=as.Date(lubridate::floor_date(as.Date(params$end.date))), ymin=-Inf, ymax=Inf)
