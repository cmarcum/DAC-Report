...
```{r include=FALSE}
dar.access.fig.cap <- sprintf("Total Data Access Requests Submitted to %s", dac)
```


```{r fig.cap=dar.access.fig.cap, echo=FALSE, warning=FALSE ,message=FALSE}
dar.request.growth.plot <- ggplot(study.status.table.dac, aes(x=Month,y=TotalRequestsCummulative)) +
...
```{r include=FALSE}
study.released.fig.cap <- sprintf("Total Number of Study Released by %s", dac)
```


```{r fig.cap=study.released.fig.cap, echo=FALSE, warning=FALSE ,message=FALSE}
study.released.growth.plot <- ggplot(study.status.table.dac, aes(x=Month,y=StudiesReleasedCummulative)) +
  geom_line(size=2, color="cornflowerblue") +
  xlab("Year") +
...
# Other Comparisons
During this reporting period, study `r phs.popular.this.dac` (`r phs.popular.this.dac.study.name`) was the requested dataset from the `r dac` DAC with `r phs.popular.this.dac.total.request` requests. 
...
# total.request.plot <- ggplot(study.status.table.dac, aes(x=Month)) +
#   geom_line(aes(y=TotalRequestsCummulative), size=2, color="cornflowerblue")
# total.studies.plot <- ggplot(study.status.table.dac, aes(x=Month)) +
#   geom_line(aes(y=StudiesReleasedCummulative), size=2, color="coral3")
# grid.arrange(total.request.plot, total.studies.plot, ncol=2)

# obj1 <- xyplot(TotalRequestsCummulative ~ Month, study.status.table.dac, type = "l" , lwd=2, col="cornflowerblue")
# obj2 <- xyplot(StudiesReleasedCummulative ~ Month, study.status.table.dac, type = "l", lwd=2, col="coral3")
#  
# # --> Make the plot with second y axis:
# doubleYScale(obj1, obj2,  text = c("TotalRequestsCummulative", "StudiesReleasedCummulative"), add.ylab2 = TRUE, use.style=FALSE )
```
